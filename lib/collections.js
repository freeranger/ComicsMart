...
Requests.after.insert(function (userId, request) {
    ...
    if (request.type == 'Buy') {
        Meteor.call('getMatchingSellRequests', request, function(err, matches) {
            if (matches.length == 0) {
        ...
        Meteor.call('getMatchingBuyRequests', request, function(err, matches) {
            if (matches.length == 0) {
...
Meteor.methods({
    ...
    getMatchingBuyRequests: function(request) {
    ...
    getMatchingSellRequests: function(request) {
        var matches = Requests.find({ isActive: true,
            type: 'Sell',
            publisher: request.publisher,
            title: request.title,
